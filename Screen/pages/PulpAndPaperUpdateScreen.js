...
const PulpAndPaperUpdateScreen = ({route, navigation}) => 
{
    ...
    const directToSurveyQuestion = (SurveyID) => {    
      navigation.navigate('SurveyQuestion', {SurveyID:SurveyID})
              ...
              <View style={{flex: 2,}}>
                <View style={{flex: 1, flexDirection: "row", marginTop:10, marginBottom:10,}}>
                  <View style={{flex: 1, paddingLeft: 10, marginRight:100}}>
                    <Text style={{ fontSize:12, color:'#000000', fontFamily:'HelveticaNeue-Bold'}}>{item.SurveyTitle}</Text>
                  </View>
                  <View style={{flex: 1, position:'absolute', right:10,top:0,}}>
                    <TouchableOpacity
                      style={styles.buttonStyle}
                      activeOpacity={0.5}
                      onPress={ () => { directToSurveyQuestion(item.SurveyID) } }
                    >
                      <Text style={styles.buttonTextStyle}>Join Now</Text>
                    </TouchableOpacity>
                  </View>
                </View>
                <View style={{flex: 2, marginBottom:3, marginTop:25}}>
                  <View style={{flex: 1, paddingLeft: 10}}>
                    <Text style={{fontSize:13, color:'#000000', fontFamily:'HelveticaNeue-Bold', opacity:0.6, marginRight:5,}}>{item.SurveyDescription}</Text>
                  </View>
                  <View style={{flex: 2, paddingLeft: 10, flexDirection:'row',marginTop:8}}>
                    <View style={{flex:1}}>
                      <Text style={{fontSize:12, color:'#000000',textAlign:'left',fontFamily:'HelveticaNeue'}}>Time to Complete</Text>
                    </View>                    
                    <View style={{flex:1}}>
                      <Text style={{fontSize:12, color:'#000000',textAlign:'right',fontFamily:'HelveticaNeue', marginRight:10}}>{item.SurveyDuration} mins</Text>
                    </View>
                  </View>                  
                </View>
              </View>
    ...
    return (
      ...
      <View style={{ alignItems: 'center', position:'absolute', top:10, left:0, right:0 }}>
          ...
          <Text style={{ marginTop:7,color:'#FDFDFD',fontSize:(fontScale < 1.2 ? 19 : 20/fontScale), fontFamily:'HelveticaNeue-Bold' }}>Market Survey</Text>
      </View>
      {
      itemList.length == 0 
      ?
        ...
        >
          <View>
            <TouchableOpacity
              ...
              onPress={() => {
                navigation.navigate('PulpAndPaperUpdate',{notificationText:'',status:''});
              }}
            > 
              <AntDesign 
                name="closesquare" 
...
export default PulpAndPaperUpdateScreen;